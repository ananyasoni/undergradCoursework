...
void* mm_malloc(size_t size) {
  ...
  // Search for free block of sufficient size
  ...
  // Get the block size and preceding block use tag
  block_size = SIZE(ptr_free_block->size_and_tags);
  preceding_block_use_tag = ptr_free_block->size_and_tags & TAG_PRECEDING_USED;
  // Check if block large enough to be split
  if ((block_size - req_size) >= MIN_BLOCK_SIZE) {
    // Split block
    block_info* remainder_block = (block_info*)UNSCALED_POINTER_ADD(ptr_free_block, req_size);
    // Set the size and tags for the remainder block
    remainder_block->size_and_tags = (block_size - req_size) | TAG_PRECEDING_USED;
    // Set the footer of the remainder block
    *((size_t*)UNSCALED_POINTER_ADD(remainder_block, block_size - req_size - WORD_SIZE)) = remainder_block->size_and_tags;
  ...
  // Mark the next block as having preceding block used
  block_info* next_block = (block_info*)UNSCALED_POINTER_ADD(ptr_free_block, block_size);
  if (next_block < (block_info*)mem_heap_hi()) {
...
void mm_free(void* ptr) {
  ...
  *((size_t*)UNSCALED_POINTER_ADD(block_to_free, payload_size - WORD_SIZE)) = block_to_free->size_and_tags;
  // Update the next block's TAG_PRECEDING_USED bit
  following_block = (block_info*)UNSCALED_POINTER_ADD(block_to_free, payload_size);
  if (following_block < (block_info*)mem_heap_hi()) {
    following_block->size_and_tags &= ~TAG_PRECEDING_USED;
    // Update footer of the following block if it's also free
    if (!(following_block->size_and_tags & TAG_USED)) {
      size_t following_size = SIZE(following_block->size_and_tags);
      *((size_t*)UNSCALED_POINTER_ADD(following_block, following_size - WORD_SIZE)) = following_block->size_and_tags;
    }
  }
